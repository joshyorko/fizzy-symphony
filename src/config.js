...
export async function loadConfig(configPath, options = {}) {
  ...
  if (extension !== ".json") {
    throw new FizzySymphonyError(
      ...
      "Task 1 parses JSON config files only; annotated YAML generation is supported for setup output.",
  ...
  try {
    ...
    throw new FizzySymphonyError("CONFIG_PARSE_ERROR", `Unable to parse config JSON: ${error.message}`, {
      path: configPath
    });
  }
