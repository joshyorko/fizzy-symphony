...
export function discoverGoldenTicketRoutes(boards, options = {}) {
  ...
  for (const board of boards) {
    ...
    for (const card of routeCards) {
      ...
      const route = parseGoldenTicketRoute(board, card, options);
...
function validateCompletionGraph(routes) {
  ...
  const routedSources = new Set(routes.map((route) => route.source_column_id));

  for (const route of routes) {
    if (route.completion.policy === "move_to_column" && routedSources.has(route.completion.target_column_id)) {
      moveTargetBySource.set(route.source_column_id, route.completion.target_column_id);
    }
  }
...
function routeOptionsFromConfig(config) {
  ...
  return {
    defaults: firstBoard?.defaults,
    allowed_card_overrides: firstBoard?.defaults?.allowed_card_overrides ?? defaultAllowedCardOverrides,
    unknownManagedTagPolicy: firstBoard?.defaults?.unknown_managed_tag_policy,
