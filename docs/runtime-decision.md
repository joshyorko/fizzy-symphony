...
Best if the priority is fastest reuse and current SDK convenience: Popper reuse, npm packaging,
...
explicit discipline. TypeScript SHOULD win only if Popper reuse or SDK convenience is decisive.
...
3. TypeScript only if Popper reuse or the current official Codex SDK is the deciding factor.
